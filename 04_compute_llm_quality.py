...
def llm_repair(grdg_node):
    ...
    for node in json.loads(grdg_node_ids):
        ...
        nodess.append(nn)
    constraint= grdg_node['constraint']
    f1s= json.loads(grdg_node['f1s'])
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dt', type=float, required=True, help='DIfficulty metric')
    ...
    df_nodes = df_nodes.sample(n=min(100, df_nodes.height))
    ...
    for node in df_nodes.iter_rows(named=True):
        ...
        # difficulty = node.select('difficulty').to_numpy()[0][0]
