...
def main():
    ...
    metric = args.metric
    budget = args.budget
    outpath = f"{dataset}/gap_{metric}_{str(theta)}_{str(budget)}/"
    ...
    for t in range(args.iters):
        ...
        # capacity repair (pair-free)
        assign_feas = repair_capacity_pairfree(assign, users_sorted_df, uid, skills, costs, CC_list, viols, lam, metric)

        # metrics
        spent  = float(assign["cost"].sum())             # relaxed spend (pre-repair)
        spentF = float(assign_feas["cost"].sum())        # feasible spend (same here)
        qualF  = float(assign_feas["p"].sum())
        # feasibility: budget only here
        feas_budget = spentF <= args.budget + 1e-12
        
        if feas_budget:
            if (best is None) or (qualF > best["quality"] + 1e-12):
                best = {"iter": t, "quality": qualF, "spent": spentF, "lambda": lam}
                ...
                if (UCB_CERT- best["quality"])/UCB_CERT < 0.01:
                    certified01 = True
                    print("Stopping early: 1% gap to UCB_CERT")
                    t4 = time.time()
                    times2.append(t4-t3)
                    break
        ...
        times2.append(t4-t3)
        if t == args.iters - 1:
            if (UCB_CERT- best["quality"])/UCB_CERT < 0.05:
                certified05 = True
                print("Stopping early: 5% gap to UCB_CERT")
        
    time1 = np.mean(times1)
    time2 = np.mean(times2)            
    pl.DataFrame(history).write_ipc(outdir / "history.feather", compression="zstd")
    (outdir / "summary.json").write_text(json.dumps({"best_feasible": best, "final_lambda": lam,
                                                     "iterations": len(history), 'certified01': certified01, 'certified05':certified05, 'UCB_CERT': UCB_CERT, 'best_lambda':best_lambda, "time1": time1,"time2":time2}, indent=2))
    
    #print(json.dumps({"best_feasible": best, "final_lambda": lam, "iterations": len(history), 'certified01': certified01, 'certified05':certified05}, indent=2))
