...
def main(file_path, output_dir):
    ...
    for edge_type in train_data.edge_types:
        ...
        if edge_index.dtype != torch.long:
            print(f"⚠️ Casting train {edge_type} edge_index to long")
            train_data[edge_type].edge_index = edge_index.long()
        if 'edge_label_index' in train_data[edge_type]:
            eli = train_data[edge_type].edge_label_index
            if eli.dtype != torch.long:
                train_data[edge_type].edge_label_index = eli.long()
    for edge_type in test_data.edge_types:
        ...
        if 'edge_label_index' in test_data[edge_type]:
            eli = test_data[edge_type].edge_label_index
            if eli.dtype != torch.long:
                test_data[edge_type].edge_label_index = eli.long()
