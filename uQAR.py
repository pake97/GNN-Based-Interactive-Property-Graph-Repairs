...
def assign_uQAR(G, users, user_model, metric):
    ...
    nodes_sorted = sorted(G.nodes, key=lambda n: G.nodes[n]['degree'], reverse=False)
    ...
    while len(nodes_sorted)>0:
        
        unlocked_nodes = [n for n in nodes_sorted]
        while len(unlocked_nodes)>0 and len(users_sorted)>0:
            chosen_node = unlocked_nodes.pop(0)  # Get the node with the lowest degree
            #for each neighbors, lock it 
            for neighbor in G.neighbors(chosen_node):
                G.nodes[neighbor]['locked'] = True
                if neighbor in unlocked_nodes:
                    unlocked_nodes.remove(neighbor)
            user = users_sorted.pop(0)
            nodes_sorted.remove(chosen_node)
            
            node_assignment[chosen_node] = user['user_id']
            user_remaining_capacity[user['user_id']] -= G.nodes[chosen_node]['difficulty']
