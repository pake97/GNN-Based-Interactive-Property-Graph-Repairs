...
class GraphGenerator:
    def __init__(self, num_users, num_tasks, tasks_dependencies, seed):
        self.num_users = num_users
        self.num_tasks = num_tasks
        self.users = [f"User{i+1}" for i in range(num_users)]
        self.tasks = [f"Task{i+1}" for i in range(num_tasks)]
        self.graph = nx.Graph()
        self.edges = []
    ...
    def generate_graph(self):
        ...
        task_dependencies = [(f"Task{i+1}", f"Task{j+1}") for i in range(int(self.num_tasks/2)) for j in range(int(self.num_tasks/2)+1, self.num_tasks) if random.random() < self.tasks_dependencies]
        self.tasks_dependencies = task_dependencies
        for t1, t2 in task_dependencies:
            self.graph.add_edge(t1, t2, dependency=True)
    ...
    def draw_graph(self):
        ...
        nx.draw(self.graph, pos, with_labels=True, node_size=3000, node_color=["skyblue" if n in self.users else "lightgreen" for n in self.graph.nodes()])
...
# # Esempio di utilizzo della classe GraphGenerator
# num_users = 10
# num_tasks = 15
# graph_generator = GraphGenerator(num_users, num_tasks)
# graph_generator.generate_graph()
# graph_generator.draw_graph()